...
{int k;
    ...
    if (t2==NULL) {t1->next=NULL; return 0;}
    else t1->next=t2;
    return 0;
}
...
{LISTA succ;
    ...
    if (succ==NULL) return -1;    /* fine lista
                                   l ora punta al nodo che precede quello da eliminare (puntato da succ)*/
    l->next = succ->next;
    free(succ);
    return 1;
}
...
int Lunghezza(LISTA l)
    {int i;
    if (l == NULL) {printf("Attenzione: lista mancante \n"); return -1;}
    l = l->next;
    i = 0;
    while (l != NULL)
    {l = l->next; i++;}
...
{if (l == NULL) {printf("Attenzione: lista mancante \n"); return -1;}
    l = l->next;
    if (l == NULL) {printf("Attenzione: lista vuota\n"); return -1;}
    printf("%d",l->dato); l = l->next;
    while (l != NULL)
    {printf("->%d",l->dato); l = l->next;}
    printf("\n");
    return 0;
